...
struct ReadOptions {
  ...
  bool force_skip_memtable = false; // for benchmark only
...
struct WriteOptions {
  ...
  // If true, write operations will be handled by GDS (GPU Direct Storage)
  // instead of the regular WAL write path. When enabled, the normal WAL
  // write at line 1687 in db_impl_write.cc will be skipped.
  //
  // Default: false
  bool write_by_gds = false;
