...
bool Reader::ReadRecord(Slice* record, std::string* scratch,
  ...
  for (;;) {
    ...
    // fprintf(stderr, "[DEBUG] log_reader ReadRecord: record_type=%u\\n", 
    //         static_cast<unsigned>(record_type));
    switch (record_type) {
      case kFullType:
      case kRecyclableFullType:
      case kNoChecksumFullType:
      case kRecyclableNoChecksumFullType:
      ...
      case kNoChecksumFirstType:
      case kRecyclableNoChecksumFirstType:
        if (in_fragmented_record && !scratch->empty()) {
          // Handle bug in earlier versions of log::Writer where
          // it could emit an empty kFirstType record at the tail end
          // of a block followed by a kFullType or kFirstType record
      ...
      case kNoChecksumMiddleType:
      case kRecyclableNoChecksumMiddleType:
      ...
      case kNoChecksumLastType:
      case kRecyclableNoChecksumLastType:
        if (!in_fragmented_record) {
          ReportCorruption(fragment.size(),
...
uint8_t Reader::ReadPhysicalRecord(Slice* result, size_t* drop_size,
  ...
  while (true) {
    ...
    // Always use standard header format: crc (4) + length (2) + type (1) [+ log_number (4)]
    const uint32_t a = static_cast<uint32_t>(header[4]) & 0xff;
    const uint32_t b = static_cast<uint32_t>(header[5]) & 0xff;
    type = static_cast<uint8_t>(header[6]);
    length = a | (b << 8);
    header_size = kHeaderSize;
    
    const bool is_recyclable_type =
        ((type >= kRecyclableFullType && type <= kRecyclableLastType) ||
         (type >= kRecyclableNoChecksumFullType && type <= kRecyclableNoChecksumLastType) ||
    ...
    // Validate log number for recyclable types
    if (is_recyclable_type) {
    ...
    // Check crc (skip for no-checksum record types 140-211)
    bool is_no_checksum = (type >= kNoChecksumFullType && type <= kRecyclableNoChecksumLastType);
    if (checksum_ && !is_no_checksum) {
    ...
    if (!uncompress_ || type == kSetCompressionType ||
      ...
      *result = Slice(header + header_size, length);
      ...
      //   fprintf(stderr, "[DEBUG] log_reader ReadPhysicalRecord: type=%u, uncompress=false, payload_size=%zu\n",
      //           static_cast<unsigned>(type), result->size());
      //   fprintf(stderr, "[DEBUG] log_reader payload: ");
      //   size_t print_len = std::min(result->size(), static_cast<size_t>(32));
      //   for (size_t i = 0; i < print_len; ++i) {
      //     fprintf(stderr, "%02x ", static_cast<unsigned char>((*result)[i]));
      //   }
      //   if (result->size() > print_len) {
      //     fprintf(stderr, "...");
      //   }
      //   fprintf(stderr, "\n");
      // }
      ...
      *result = Slice(uncompressed_record_);
      ...
      // add debug log to print the result
      // if (result && result->size() > 0) {
      //   fprintf(stderr, "[DEBUG] log_reader ReadPhysicalRecord: type=%u, uncompress=true, payload_size=%zu\n",
      //           static_cast<unsigned>(type), result->size());
      //   fprintf(stderr, "[DEBUG] log_reader payload: ");
      //   size_t print_len = std::min(result->size(), static_cast<size_t>(32));
      //   for (size_t i = 0; i < print_len; ++i) {
...
bool FragmentBufferedReader::TryReadFragment(Slice* fragment, size_t* drop_size,
  ...
  // First, ensure we have enough bytes for the standard header
