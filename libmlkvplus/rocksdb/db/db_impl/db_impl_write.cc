...
IOStatus DBImpl::WriteToWAL(const WriteBatch& merged_batch,
  ...
  *log_size = log_entry.size();
  ...
  // Skip WAL write if write_by_gds is enabled
  if (write_options.write_by_gds) {
    // When write_by_gds is enabled, skip the regular WAL write
    // and let GDS (GPU Direct Storage) handle the write operation
    io_s = IOStatus::OK();
  } else {
...
IOStatus DBImpl::WriteGroupToWAL(const WriteThread::WriteGroup& write_group,
  ...
  write_options.write_by_gds = write_group.leader->write_by_gds;
