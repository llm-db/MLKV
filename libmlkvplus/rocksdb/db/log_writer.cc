...
Writer::Writer(std::unique_ptr<WritableFileWriter>&& dest, uint64_t log_number,
    ...
    : dest_(std::move(dest)),
      ...
      wal_with_checksum_(wal_with_checksum) {
...
IOStatus Writer::AddRecord(const WriteOptions& write_options,
  ...
  if (s.ok()) {
    do {
      ...
      if (!wal_with_checksum_) {
        // Use no-checksum record types (140-211)
        if (begin && end) {
          type = recycle_log_files_ ? kRecyclableNoChecksumFullType : kNoChecksumFullType;
        } else if (begin) {
          type = recycle_log_files_ ? kRecyclableNoChecksumFirstType : kNoChecksumFirstType;
        } else if (end) {
          type = recycle_log_files_ ? kRecyclableNoChecksumLastType : kNoChecksumLastType;
        } else {
          type = recycle_log_files_ ? kRecyclableNoChecksumMiddleType : kNoChecksumMiddleType;
        }
      } else {
        // Use traditional checksum record types
        if (begin && end) {
      ...
      // add debug log
      // fprintf(stderr, "[DEBUG] log_writer AddRecord: type=%u, fragment_length=%zu\\n", 
      //         static_cast<unsigned>(type), fragment_length);
...
IOStatus Writer::EmitPhysicalRecord(const WriteOptions& write_options,
  ...
  // add debug log
  if (kDebugInfo) {
    fprintf(stderr, "[DEBUG] Starting log_writer EmitPhysicalRecord: type=%u, size=%zu\n", 
  ...
  // Check if this is a no-checksum record type (140-211)
  bool is_no_checksum = (t >= kNoChecksumFullType && t <= kRecyclableNoChecksumLastType);

  // Always use standard header format
  buf[4] = static_cast<char>(n & 0xff);
  buf[5] = static_cast<char>(n >> 8);
  buf[6] = static_cast<char>(t);

  uint32_t crc = 0;  // Initialize CRC
  if (t < kRecyclableFullType || t == kSetCompressionType ||
      t == kPredecessorWALInfoType || t == kUserDefinedTimestampSizeType || (t >= kNoChecksumFullType && t <= kNoChecksumLastType)) {
    // Legacy record format
    assert(block_offset_ + kHeaderSize + n <= kBlockSize);
    header_size = kHeaderSize;
  } else {
    // Recyclable record format (including no-checksum recyclable types)
    assert(block_offset_ + kRecyclableHeaderSize + n <= kBlockSize);
    header_size = kRecyclableHeaderSize;

    // Only encode low 32-bits of the 64-bit log number.
    EncodeFixed32(buf + 7, static_cast<uint32_t>(log_number_));
    if (!is_no_checksum) {
      crc = crc32c::Extend(type_crc_[t], buf + 7, 4);
    }
  }

  if (!is_no_checksum) {
    // Compute the crc of the record type and the payload for checksum types
  ...
  // For no-checksum types, CRC remains 0
  if (is_no_checksum) {
    buf[0] = 0;
    buf[1] = 0;
    buf[2] = 0;
  ...
  if (kDebugInfo) {
    // print the header and payload for debug
    fprintf(stderr, "[DEBUG] log_writer EmitPhysicalRecord: type=%u, header_size=%zu, payload_size=%zu\n", 
            static_cast<unsigned>(t), header_size, n);
    fprintf(stderr, "[DEBUG] log_writer header: ");
    for (size_t i = 0; i < header_size; ++i) {
      fprintf(stderr, "%02x ", static_cast<unsigned char>(buf[i]));
    }
    fprintf(stderr, "\n");
    fprintf(stderr, "[DEBUG] log_writer payload: ");
    size_t print_len = std::min(n, static_cast<size_t>(32));
    for (size_t i = 0; i < print_len; ++i) {
      fprintf(stderr, "%02x ", static_cast<unsigned char>(ptr[i]));
    }
    if (n > print_len) {
      fprintf(stderr, "...");
    }
