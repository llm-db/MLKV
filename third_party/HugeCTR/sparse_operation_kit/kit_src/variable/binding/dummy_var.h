...
//
// It should be the only interface for PyTorch op to access the code
// under variable/impl/, to isolate the sok:: code from PyTorch code.
...
class DummyVar : public torch::CustomClassHolder {
 ...
 private:
  ...
  std::string name_;
  std::mutex mu_;  // Use std::mutex instead of TensorFlow mutex
